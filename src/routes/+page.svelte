<script>
  ...
  function initializeEmptyRows() {
    ...
    editingContacts = [...contacts.map(c => ({ ...c, isNew: false, isModified: false })), emptyRow];
  }
  ...
  async function saveAllChanges() {
    ...
    try {
      ...
      const contactsToSave = editingContacts.filter(contact => {
        return (contact.isModified || contact.isNew) && 
               contact.name.trim() && 
               contact.email.trim();
      }).map(contact => {
  ...
  $: modifiedCount = editingContacts.filter(c => c.isModified || (c.isNew && (c.name.trim() || c.email.trim()))).length;
...
<div class="container">
  ...
  <section class="control-panel">
    ...
    <div class="actions">
      <button on:click={addMoreRows} class="btn-secondary">
        + Add Row
      </button>
        ...
        <table class="contacts-table">
          ...
          <tbody>
            {#each editingContacts as contact, index}
              <tr class="contact-row" class:new-row={contact.isNew} class:modified-row={contact.isModified}>
                <td class="col-name">
                  <input
